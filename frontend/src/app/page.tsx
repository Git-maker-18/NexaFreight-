...
function DashboardInner() {
  ...
  useEffect(() => {
    ...
    // Polling — news every 30 min
    const intervals = [
      setInterval(() => fetchEndpoint('/api/news', undefined, undefined, { skipWhenHidden: true }), 1800000),
    ];
    return () => {
  ...
  return (
    <main className="fixed inset-0 w-full h-full bg-[var(--bg-void)] overflow-hidden">
        ...
        {showSplash && (
          ...
          >
            ...
            <div className="flex items-center gap-[2px] mb-3 z-[2]">
              {'NEXAFREIGHT'.split('').map((letter, i) => (
                <motion.span
                  key={i}
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 2.5 }} className={`absolute top-4 z-[200] pointer-events-none flex flex-col`} style={{ left: isMobile ? '24px' : '64px', right: '24px' }}>
        <div className="flex items-center gap-3 w-fit">
          ...
          <div className="flex flex-col items-start gap-0.5">
            <h1 className="text-lg md:text-xl font-bold tracking-[0.4em] text-[#D4AF37] font-mono">NEXAFREIGHT</h1>
        ...
        <div className="flex items-center gap-3 mt-1.5 pl-[44px] min-w-0 pr-4">
          <span className="text-[9px] md:text-[9px] text-[var(--text-muted)] font-mono tracking-[0.2em] md:tracking-[0.3em] uppercase opacity-40 truncate">
            REAL-TIME GLOBAL MONITORING <span className="hidden md:inline">· FLIGHTS · MARITIME · WEATHER · NEWS · CYBER THREATS</span>
          </span>
        </div>
      </motion.div>
      ...
      {showLayers && !isMobile && <LayerPanel data={data} activeLayers={activeLayers} setActiveLayers={setActiveLayers} theme={globeTheme} setTheme={setGlobeTheme} capabilities={capabilities} />}
...
/**
 * AuthGate — holds the auth guard logic separately so DashboardInner's hooks
 * are always called unconditionally (Rules of Hooks compliance).
 *
 * Step 8: redirects to /login when no token.
 * Step 9: logs getCurrentUser() and getShipments() to the browser console
 *         after a successful login to verify end-to-end connectivity.
 *         REMOVE the Step 9 useEffect once you have confirmed the data.
 */
function AuthGate() {
  const router = useRouter();
  const { isAuthenticated, isHydrated, clearAuth } = useAuthStore();

  // Redirect unauthenticated visitors to /login once client storage is hydrated
  useEffect(() => {
  ...
  // Step 9: end-to-end verification with graceful 401 expiration handling
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        console.log('[NexaFreight] getCurrentUser() →', user);
      } catch (err: any) {
        if (cancelled) return;
        console.error('[NexaFreight] getCurrentUser() failed:', err);
      ...
      try {
        const ships = await getShipments({ size: 5 });
        if (cancelled) return;
        console.log('[NexaFreight] getShipments() →', ships);
        if (ships.items.length === 0) {
          console.warn(
            '[NexaFreight] getShipments() returned 0 items. ' +
            'Check that T-016/T-017 ingestion scripts ran against this DB.'
          );
        }
      } catch (err: any) {
        if (cancelled) return;
        console.error('[NexaFreight] getShipments() failed:', err);
        if (err?.status === 401 || err?.statusCode === 401) {
          clearAuth();
          router.replace('/login?reason=expired');
        }
  ...
  }, [isHydrated, isAuthenticated, clearAuth, router]);

  // Ensure SSR and client initial render match 100% until hydrated (eliminates hydration error)
  if (!isHydrated) {
    return (
      <main className="fixed inset-0 w-full h-full bg-[var(--bg-void)] flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          <div className="w-8 h-8 rounded-full border-2 border-[var(--gold-primary)] border-t-transparent animate-spin" />
          <span className="font-mono text-xs text-[var(--gold-light)] tracking-widest">
